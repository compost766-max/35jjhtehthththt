...
@user.callback_query(F.data == 'privat')  
async def check_brand(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(text="🔥 Выбери приват который хочешь купить :", reply_markup=kb.privaty)
...
@user.callback_query(F.data == 'aeprivs')  
async def check_brand(callback: CallbackQuery):
    await callback.answer(f'Скоро...', show_alert=True)
    
    
@user.callback_query(F.data == 'editpriv')  
async def check_brand(callback: CallbackQuery):
    await callback.answer(f'Скоро...', show_alert=True)
    
    
@user.callback_query(F.data == 'vdnodes')  
async def check_brand(callback: CallbackQuery):
    await callback.answer(f'Скоро...', show_alert=True)
...
@user.callback_query(F.data == 'editback')  
async def back(callback: CallbackQuery):
    await callback.answer()
    await callback.message.edit_text(text="🔥 Выбери, интересующую тебя, категорию товара:", reply_markup=kb.price)
